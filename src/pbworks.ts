...
class PBWorks {
     ...
     */
    async putPageContent(page: string, author: string, html: string, createIfMissing: boolean = true): Promise<boolean> {
     ...
     */
    async getUserInfo(email:string): Promise<object>{
        return await this.operationGet("GetUserInfo", new Map(Object.entries({
            email: email
        })));
    }
}
